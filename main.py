...
pygame.display.set_caption("Behula:The Game")
...
# class for creating and moving obstacle enemy
class Enemy:

    def __init__(self, speed=10):

        self.enemy_images = load_sprites("image/enemy/", "enemy_", 5, 160, 160)

        self.enemy_image_0, self.rect_0 = self.enemy_images[0], self.enemy_images[0].get_rect()
        self.enemy_image_1, self.rect_1 = self.enemy_images[1], self.enemy_images[1].get_rect()
    ...
    def get_enemy(self):
        ...
        current_enemy = [self.enemy_image_0, self.enemy_image_1]
        enemy_rect = [self.rect_0, self.rect_1]

        return current_enemy, enemy_rect
    ...
    def draw(self):
        ...
        window.blit(self.enemy_image_0, self.rect_0)
        window.blit(self.enemy_image_1, self.rect_1)
    ...
    def update(self):
        ...
        if self.rect_0.right < 0:
            ...
            self.enemy_image_0 = self.enemy_images[temp_index]
        ...
        if self.rect_1.right < 0:
            ...
            temp_index = random.randrange(0, 5)
            self.enemy_image_1 = self.enemy_images[temp_index]


# class for creating and moving our Behula
class Behula:

    def __init__(self):

        self.idle_images = load_sprites("image/behula/", "idle_", 10, 220, 153)
        self.running_images = load_sprites("image/behula/", "run_", 8, 220, 153)
        self.jumping_images = load_sprites("image/behula/", "jump_", 16, 220, 153)
        ...
        # these indexes cycle through the images of the sprites, make the behula look moving
        self.idle_index = 0
    ...
    def check_collision(self, all_enemy):

        if self.running:
            behula_mask = pygame.mask.from_surface(self.running_images[self.running_index])
        elif self.jumping:
            behula_mask = pygame.mask.from_surface(self.jumping_images[self.jumping_index])
        else:
            behula_mask = pygame.mask.from_surface(self.idle_images[self.idle_index])

        current_enemy, enemy_rect = all_enemy

        offset_0 = (enemy_rect[0].left - self.rect.left, enemy_rect[0].top - self.rect.top)
        offset_1 = (enemy_rect[1].left - self.rect.left, enemy_rect[1].top - self.rect.top)

        collide = behula_mask.overlap(pygame.mask.from_surface(current_enemy[0]), offset_0) or \
            behula_mask.overlap(pygame.mask.from_surface(current_enemy[1]), offset_1)
...
class Score:
    ...
    def draw(self):
        ...
        draw_text(str(self.high_score), "font/monofonto.ttf", 28, (255,223,0), SCREEN_WIDTH - 60, 20, "topright")
        draw_text(str(self.score), "font/monofonto.ttf", 28, (255,223,0), SCREEN_WIDTH - 60, 65, "topright")
    ...
    while run:
        ...
        if key[K_SPACE] or key[K_UP]:
            ...
            if game_over:
                play_again = True
                run = False
            elif not behula.jumping:
                jump_sound.play()
                behula.jumping = True
                behula.running = False
        ...
        if game_over:
            ...
            if behula.check_collision(enemy.get_enemy()):
                game_over = True
                game_over_screen.draw()
